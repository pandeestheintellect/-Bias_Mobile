...
import DialogAddExternalWall from "../components/dialogs/dialog-add-external-wall";
...
class AssessmentExternalWall extends Component<RouteComponentProps<TParams> & ProviderContext,IState> {
    ...
    constructor(props:RouteComponentProps<TParams> & ProviderContext) {
        ...
        this.onWallDownloaded = this.onWallDownloaded.bind(this);
    ...
    onProjectHeaderLoad(headerInfo:IProject)
    {
        ...
        let name =  headerInfo.name===undefined?'External Wall':'External Wall - '+headerInfo.name;
    ...
    onModuleProcessMasterDownloaded(moduls:IAssessmentModuleProcess[]|string)
    {
        if(Array.isArray(moduls))
        {
            ...
            {
                if (process[i].assessmentId===AssessmentTypes.ExternalWall)
                {
                    if(mst!==undefined)
                    {
    ...
    onLocationMasterDownloaded(moduls:IAssessmentLocation[]|string)
    {
        if(Array.isArray(moduls))
        {
            ...
            getExternalWall(this.projectId,this.onWallDownloaded)
        }
        else
            this.onUpdateStatus(moduls as string)
    }
    onWallDownloaded(wall:IExternalWall[]|string)
    {
        if(Array.isArray(wall))
        {
            ...
            this.rowFullList=JSON.parse(JSON.stringify(wall));
        ...
        //removeSyncUploadDetails(this.projectId,this.state.projectInfo?.name as string,'External Wall'); 
    ...
    onSearch(keyword:string){
        ...
        else
        {
            ...
            let lst = this.rowFullList.filter((item:IExternalWall)=>{
    ...
    onHeaderClick(action:string, path:string){
        if (action==='BACK')
            ...
            this.props.history.push('/assessment-sync/external-wall/'+this.projectId);  
        else if (action==='ADD')  
        {
    ...
    onUpdateStatus(status:string)
    {
        ...
        if(status==='All data uploaded')
            downloadExternalWall(this.projectId, this.onWallDownloaded);
    }
    ...
    onProcessChange(cardIndex: number,id: number,processId: number,rowIndex: number,colIndex: number,oldStatus:number, currentStatus:number){
        ...
        this.setState({noOfCompliance:this.compliance,noOfCheck:this.checks,
            noOfProcessCompliance:compliance,noOfProcessCheck:checks,isEdited:true});
        this.onUpdateSummary(this.compliance,this.checks,compliance,checks);

        markForUpdate( {moduleId:AssessmentTypes.ExternalWall,
            moduleName:'External Wall',
            projectId:this.projectId,
            projectName:this.state.projectInfo?.name as string,
            key:'external-wall-'+this.projectId
         },this.rowFullList);
    ...
    onUpdateSummary(noOfCompliance:number, noOfCheck:number,compliance:number[],checks:number[])
    {
        if (noOfCheck===0)
        {
            updateAssessmentWeightage(this.projectId,{typeId:AssessmentTypes.ExternalWall,
                moduleId: 8,moduleName: '',compliances:0,checks:0,percentage:0,weightage:0,score:0,nonCompliance:''})
            return;
        }
        ...
        updateAssessmentWeightage(this.projectId,{typeId:AssessmentTypes.ExternalWall,
            moduleId: 8,moduleName: '',compliances:noOfCompliance,checks:noOfCheck,
    ...
    onDelete (id: number){
        ...
        let deleted:IExternalWall=  this.rowList[id];
        let compliance = this.state.noOfProcessCompliance;
        let checks = this.state.noOfProcessCheck;
        ...
        if(deleted.status!==1)
        {
            markForDeletion( {moduleId:AssessmentTypes.ExternalWall,
                moduleName:'External Wall',
                projectId:this.projectId, 
                projectName:this.state.projectInfo?.name as string,
                key:'external-wall-deleted-'+this.projectId
             },deleted.id);
    ...
    canAdd(rowList:IExternalWall[],blockUnit:string,date:string,locationId:number)
    {
    ...
    onCreate(blockUnit:string,date:string,locationId:number,location:string)
    {
        ...
        let walldetail:IExternalWall = {id:this.rowFullList.length+1,date:date,block:blockUnit,
        ...
        this.state.process?.process.forEach((pro:IProcess,index:number) => {
                ...
                [0,1,2,3].forEach((rowNo:number)=>{
                    walldetail.details.push({id:0,pId:pro.id,
        ...
        this.rowFullList.push(walldetail);
        this.rowList.push(walldetail);
        this.compliance +=20;
        this.checks +=20;
    ...
    getRow(rowDetail:IExternalWall){
        return (
            <div style={{display:'flex',flexDirection:'row',height:'92px',borderBottom:'1px solid gray'}}>
                <div style={{width:'145px', minWidth:'145px',borderRight:'1px solid gray'}}>
                {
                ...
                <div>
                    <div style={{display:'flex',flexDirection:'row',height:'30px',width:'100%',borderBottom:'1px solid gray'}}>
                        ...
                        <div style={{width:this.state.winWidth-560,paddingTop:'2px',paddingLeft:'4px', borderRight:'1px solid gray'}}>{rowDetail.date}</div>
                        <div style={{width:'50px',paddingTop:'2px',paddingLeft:'4px'}}>W 2</div>   
                    </div>
                    <div style={{display:'flex',flexDirection:'row',height:'30px',width:'100%',borderBottom:'1px solid gray'}}>
                        <div style={{width:'70px',paddingTop:'2px',paddingLeft:'4px', borderRight:'1px solid gray'}}>Block</div> 
                        <div style={{width:this.state.winWidth-560,paddingTop:'2px',paddingLeft:'4px', borderRight:'1px solid gray'}}>{rowDetail.block}</div>
                        <div style={{width:'50px',paddingTop:'2px',paddingLeft:'4px'}}>W 3</div>   
                    </div>
                    <div style={{display:'flex',flexDirection:'row',height:'30px',width:'100%',borderBottom:'1px solid gray'}}>
                        <div style={{width:'70px',paddingTop:'2px',paddingLeft:'4px', borderRight:'1px solid gray'}}>Location</div> 
                        <div style={{width:this.state.winWidth-560,paddingTop:'2px',paddingLeft:'4px', borderRight:'1px solid gray'}}>{rowDetail.lName}</div>
                        <div style={{width:'50px',paddingTop:'2px',paddingLeft:'4px'}}>W 4</div>   
                    </div>
                </div>
            </div>
        );
                ...
                <div className='assessment-unit unit-row' 
                    ...
                    {backgroundColor:'#fff',height:'100%',display:'flex',flexDirection:'row',width:this.state.winWidth-30}}>
                        <div className='unit-one-col1' style={{width:this.state.winWidth-280}}>
                            {this.getFirstRow(row.index,(row.index+1)+'','W 1')}
                            {this.getRow(rowData)}
                        </div>
        ...
        return (
            ...
            <div className={'page'}>
                ...
                <div className='assessment-unit unit-row MuiPaper-elevation4' 
                    style={{marginTop:'10px',height:'173px',display:'flex',flexDirection:'row',width:this.state.winWidth-30}}>
                ...
                <div style={{height:this.state.winHeight-330,width:this.state.winWidth-10}} >
                <AutoSizer>
                    {({ height, width }) => (
                        <List
                ...
                <DialogAddExternalWall ref={this.addDialog} onCreate={this.onCreate} />   
                <DialogDeleteConfirm ref={this.deleteDialog} onDelete={this.onDelete} />  
                <DialogSync ref={this.syncDialog} projectId={this.projectId} module={'External Wall'} onSync={this.onSync}/>
                <Prompt
                    when={true}
...
export default withRouter(withSnackbar(AssessmentExternalWall))
